...
kliens.on('message', async (message) => 
{
    ...
    try {
      let commandFile = require(`./comhand/${command}.js`);
      commandFile.run(kliens, message, args, prefix, moment);
    } catch (err) {
}});
...
kliens.on('message', message => {
  if (message.content.startsWith('addrole')) {
    ...
    let myRole = message.guild.roles.get("651856149813002253"); // developper rang
...
kliens.on("message", async message => {
  if (message.author.bot) return;
  if (message.content === prefix + "little") {
    const reacttoembed = new discord.RichEmbed()
    .setTitle("Olivér meleg ?")
    message.channel.send(reacttoembed).then(function (message) {
      message.react(`${emo[":pipa"]}`).then(
      message.react(`${emo[":X"]}`))
  }).catch(function() {
    // semmi sem kell most ide:D
  })
  }
});
...
kliens.on("message", message => {
    if (message.content.toLowerCase() === `!version`) {
      const kkenn = new discord.RichEmbed()
        ...
        .addField("**1.9*3**", "**°**Time?")
        .setFooter(`Parancsot meg hívta: ${message.author.toString()}`)
...
const tok = "Njc0OTk3NDgyNjA5OTY3MTE2.XlBldA.7QAY6A7fb8LDKm3cHLB0BttOdpk"

kliens.login('Njc0OTk3NDgyNjA5OTY3MTE2.XlBldA.7QAY6A7fb8LDKm3cHLB0BttOdpk')
